...
def save_image(image):
    ...
    image.filename = "testing.jpg"  # generate_random_string()
...
def process_image(image_path):
    ...
    image_arr = image_arr.reshape(
        (1, image_arr.shape[0], image_arr.shape[1], image_arr.shape[2])
    )
...
def process_list_image(img_list):
    ...
    ans = []
    for i in range(idx):
        ans.append(BASE_DIR + img_list[i] + ".jpg")
    return ans
...
def predict_result(image):
    ...
    predicted_caption = predict_caption(
        model, feature, tokenizer, max_length=max_length
    )
    print(predicted_caption)
    result = get_matching_strings(
        main_string=predicted_caption,
        min_keyword_matches=int(len(predicted_caption.split()) * 0.9),
    )
    result = process_list_image(result)
    mapping[image_path] = predicted_caption
