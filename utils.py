...
def generate_random_string():
    ...
    random_string = ''.join(random.choice(characters) for _ in range(10)) + ".jpg"
...
def get_matching_strings(main_string,  min_keyword_matches, target_strings = pickle.load(open("mapping.pkl", "rb"))):
    trie = Trie()
    ...
    main_string_words = [word.lower() for word in main_string.split() if word.lower() not in stopwords]

    # Insert target strings into Trie
    for target_id, target_string in target_strings.items():
        target_string_words = [word.lower() for word in target_string.strip().split() if word.lower() not in stopwords]
    ...
    matches = []
    for target_id, target_string in target_strings.items():
        target_string_words = [word.lower() for word in target_string.split() if word.lower() not in stopwords]
